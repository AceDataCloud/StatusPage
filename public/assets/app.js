(() => {
  ...
  const STATUS_CONFIG = {
    ...
    unknown: { label: 'No Data', barColor: 'bg-slate-200 dark:bg-slate-700', dotColor: 'bg-slate-300 dark:bg-slate-600', textColor: 'text-slate-500 dark:text-slate-400' }
  };
  const OVERALL = {
    ...
    no_data: ['No Recent Observations', 'bg-slate-50 dark:bg-slate-900/40', 'border-slate-200 dark:border-slate-700', 'bg-slate-400', 'text-slate-700 dark:text-slate-300', '–']
  };
  let currentDays = 1;
  ...
  function renderService(service) {
    ...
    middle.textContent = service.uptime === null ? 'No observed traffic' : `${service.uptime.toFixed(1)}% observed uptime`;
  ...
  async function load() {
    ...
    try {
      ...
      if (!data.range.services.length) {
        ...
        empty.textContent = 'No services have recent observations.';
